...
drop procedure if exists pr_div_set_postbulk_successfinacle$$

CREATE PROCEDURE pr_div_set_postbulk_successfinacle(
...
me:BEGIN
  ...
  select a.acc_no,a.val_date,a.pay_mode,a.batch_no,sum(a.div_amount) as div_sum from div_trn_tsuccess as a
  ...
  and a.delete_flag='N'
  group by a.acc_no,a.val_date,a.pay_mode,a.batch_no;
  ...
  select count(*) into n from div_trn_tsuccess as a
  where a.tran_dr_gid = 0
  and a.file_gid = if(in_file_gid > 0,in_file_gid,a.file_gid)
  and a.acc_no = if(in_acc_no <> '',in_acc_no,a.acc_no)
  and a.pay_mode <> 'W'
  ...
  open cur;
    read_loop:loop
      fetch cur into v_acc_no,v_val_date,v_pay_mode,v_batch_no,v_div_sum;
      ...
      if exists(select tran_gid from div_trn_ttran
        ...
        and mapped_amount = 0
        and tran_desc like concat('%',ifnull(v_batch_no,''),'%')
        and delete_flag = 'N') and v_count = 0 then
  ...
  call pr_div_set_post_successfinacle(in_acc_no,in_file_gid,in_system_ip,in_action_by,out_msg,out_result);

  call pr_div_ins_log('Posting Success With Finacle',in_system_ip,in_action_by);

  select count(*) into c from div_trn_tsuccess as a
  where a.tran_dr_gid = 0
  and a.file_gid = if(in_file_gid > 0,in_file_gid,a.file_gid)
  and a.acc_no = if(in_acc_no <> '',in_acc_no,a.acc_no)
  and a.pay_mode <> 'W'
