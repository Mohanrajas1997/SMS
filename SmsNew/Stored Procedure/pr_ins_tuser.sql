...
DROP PROCEDURE IF EXISTS `pr_ins_user` $$
CREATE PROCEDURE `pr_ins_user`(
  in in_user_gid int,
...
me:BEGIN
  ...
  if in_action = 'UPDATE' then
    ...
    if in_dor = '0000-00-00' then
      set v_auth_sql = concat(v_auth_sql,'dor = null,');
      set v_auth_sql = concat(v_auth_sql,'attempt_count = 0,');
      set v_auth_sql = concat(v_auth_sql,'login_date = null,');
    else
      set v_auth_sql = concat(v_auth_sql,'dor = ',char(39),date_format(in_dor,'%Y-%m-%d'),char(39),',');
    end if;

    if in_pwd_flag = true then
      set v_auth_sql = concat(v_auth_sql,'pwd =',char(39),char(39),',');
    end if;
    ...
    set v_auth_sql = concat(v_auth_sql,'user_status = ',char(39),in_user_status,char(39),' ');
