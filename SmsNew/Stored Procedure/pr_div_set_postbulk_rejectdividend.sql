...
drop procedure if exists pr_div_set_postbulk_rejectdividend$$

CREATE PROCEDURE pr_div_set_postbulk_rejectdividend(
...
me:BEGIN
  ...
  update div_trn_treject set div_gid = 0
  where div_gid in
  (
    select a.div_gid from
    (
      select div_gid,count(*) from div_trn_treject
      where true
      and file_gid = if(in_file_gid > 0,in_file_gid,file_gid)
      and acc_no = if(in_acc_no <> '',in_acc_no,acc_no)
      and div_gid > 0
  ...
  call pr_div_set_post_rejectdividend(in_acc_no,in_file_gid,in_system_ip,in_action_by,out_msg,out_result);
  ...
  call pr_div_ins_log('Posting Reject With Dividend',in_system_ip,in_action_by);
